...
Note the call to {\ttfamily .should\+Exit()} on the context -\/ that is very important -\/ it will be set when a query flag has been used (or the {\ttfamily -\/-\/no-\/run} option is set to {\ttfamily true}) and it is the user\textquotesingle{}s responsibility to exit the application in a normal way.\hypertarget{md__home_kalafiorek__programowanie_obj2021_zad5_2-_mateusz-_paszkiewicz_external_doctest_doc_markdown_main_autotoc_md119}{}\doxysubsection{Dealing with shared objects (\+D\+L\+Ls)}\label{md__home_kalafiorek__programowanie_obj2021_zad5_2-_mateusz-_paszkiewicz_external_doctest_doc_markdown_main_autotoc_md119}
